...
export class EsriMapComponent implements OnInit, OnDestroy {
  ...
  @Input() basemap = ''; // Tell the component what basemap to choose to display...
  @ViewChild('map', {read: ElementRef, static: true}) mapObj: ElementRef; // Controls the html element to put the web map..
  @ViewChild('window', {read: ElementRef, static: true}) window: ElementRef; // controls the container html of the map..
  @ViewChild('xy', {read: DisplayXYComponent, static: true}) xyComp: DisplayXYComponent;
  @ViewChildren('polygon') legendUI: QueryList<ElementRef>;
  height: any = '95%';
  ...
  isAlive = true; // Controls the alive part for unsubscribe and subscribe...
  ...
  ngOnInit() {
    ...
    this.app.actionsQuickPick.pipe(takeWhile(() => this.isAlive)).subscribe(response => {
  ...
  initMap() {
    ...
    if (this.basemap == 'MAPFLEX') {
      ...
      if (this.app.mapFlexBaseMap) {
        ...
        if (this.app.esriDynamicLayer) {
          ...
          const options = {serviceMode: 'KVP', layerInfo, id: 'wms'};
    ...
    // Once Layers have been added init the xy comp.
    if (this.xyComp) { this.xyComp.setUpMap(this.map); }
    ...
    // Add Extent Change to display quick pick..
        ...
        const extent = this.app.esriwebMercatorUtils.webMercatorToGeographic(response.extent);
      ...
      // tslint:disable-next-line:no-shadowed-variable
        this.map.on('click', response => {
          ...
          const graphic = new this.app.esriGraphic(response.mapPoint, this.pointSymbol); // temporarily hold graphic...
          ...
          this.app.GEOMETRY_METHOD('geom=' + JSON.stringify(response.mapPoint)).subscribe((response: any) => {
             ...
             if (response){
              if (response.features.length > 0) {
                ...
                const poly = new _self.app.esriPolygon(response.features[0].geometry);
                const graphic = new _self.app.esriGraphic(poly, _self.polygonSymbol);
    ...
    // =-=-=-=-=-= ENDING OF MAP EVENTS =-=-=-=-=-=-=-=-=-=-=-=
     // Check if there is graphics to display...
      ...
      const graphic = new this.app.esriGraphic(this.point, this.pointSymbol);
      ...
      const circle = this.app.esriCircle(this.point, {radius: 300});
      ...
      this.isLoading = false;
        ...
        const graphic = new this.app.esriGraphic(this.polygon, this.polygonSymbol);
  ...
  onLegend(ele: HTMLInputElement, purpose: string) {
        ...
        if (purpose === 'polygon') {
          ...
          // Loop check if anything is selected..
          // tslint:disable-next-line:no-shadowed-variable
          this.legendUI.toArray().forEach((ele) => {

            const ui = ele.nativeElement;
            // tslint:disable-next-line:radix
            const index: number = parseInt(ui.dataset.index);
            this.legendLayers.push((ui.checked ? index : -1));
          });
  ...
  maximizeMap() { // GO FULL SCREENT..
    const elem = this.window.nativeElement;
  ...
  onMeasure() {
    ...
    const style = tool.style.display;
    if (style == 'none') {
  ...
  changeBaseMap() {
      ...
      const layers = this.map.getLayersVisibleAtScale(this.map.getScale());
      const holdLayer = null;
      if (this.base) {
