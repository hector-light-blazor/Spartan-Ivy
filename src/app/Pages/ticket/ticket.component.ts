...
export class TicketComponent implements OnInit {
  ...
  showFileUploader = false; // Displays the file uploader to attach files to ticket...
  showFileViewer = false; // Displays all the attachments to this ticket...
  displayWorkflow = false; // Display all worflow for this ticket...
  connectionsCount = 0; // How many same tickets exists...
  messagesCount = 0; // Messages Telling User information is available..
  connectionsPulse = false; // Controls the css animation..
  showAttach = false; // .. This Is To Show Attachments For Ticket Component
  showViewAttach = false; // .. This Shows Attachement For Ticket Component
  showViewRoutes = false; // TODO: MayBe We Needed IDK To Show Where The Ticket Is Going lol
  showRoutes = false; // TODO: Show Routes Maybe not needed this is OLD need to remove..
  isAlive = true; // Needs this for subscribe of toolbar actions...
  comments: Array<FEED> = []; // ..  Holds The Comment Feeds For Ticket ...
  isLoading = true;
  isConnectionsLoading = true;
  numberOfDays = ''; // Shows the number days left for the ticket..
  iconNumberOfDays = false; // Displays Something...
  due_date = ''; // Date the ticket is due..
  init_date = ''; // Show beginning Date...
  front = true; // Controls display remainder days left...
  confirmSelection = -1; // display confirmation display.. for delete and archive...
  parcels: any; // List All Parcels from db server
  wparcels: any; // List All WCAD Parcels from db server..
  profileImage = 'assets/avatar.png'; // display profile image...
  isMapEnabled = false; // Controls the esri map view to display ...
  isGoogleEnabled = false; // Controls the google map view to display ...
  extent: any = null; // ticket extent from parcels...
  base = ''; // Tell the esri map component what basemap to use....
  ...
  searchParcel = ''; // find particular parcel information from db server...
  enableTicket = false; // this variable will tell esri map comp either to display graphics layer for ticket or not for editing and other purposes.
  users: Array<USER> = []; // Gets a list of all users to be used in drop downs as options...
  lvusers: any = []; // assigns the lv list users...
  susers: any  = []; // the super users list...
  confirmationName = ''; // this controls the name of what to confirm on pop up....
  stopSave = false; // NG DESTROY HACK PREVENTS FROM SENDING TICKET TO NEXT PERSON....
  displayDialog = false; // Display
  iswalking = '';
  meFullName = '';
  meFullName2 = '';
  meStamp = false;
  ...
  url_log = 'addressticket/saveLogs';
  ...
  constructor(public app: AppService, private route: ActivatedRoute, private router: Router) {
    ...
    if (this.app.account_info.config){
      // tslint:disable-next-line:prefer-for-of
        for (let x = 0; x < this.app.account_info.config.length; x++){
         ...
         if (params.hasOwnProperty('id')) {
           ...
           this.app.GET_METHOD(this.app.route.api.vwTicket + 'id=' + ticket).subscribe((response: any) => {
            ...
            if (response) {
              ...
              this.logs.downloadLogs(this.attributes.id_ticket);
  ...
  ngOnInit() {
    ...
    setTimeout(() => {
      _self.app.ticketInteractionToolbar.next({toolbar: true, data: true});
    ...
    if (this.table) { this.table.bodyHeight = 400; }

  }

  // LifeCycle Angular Framework.
  // if the component detects is going to change view or destroy do something this happens..
 ngOnDestroy() {
    ...
    this.app.ticketInteractionToolbar.next({toolbar: true, data: false});

    // Remove the un archive to archive feature...
    this.app.ticketInteractionToolbar.next({archive: true, data: false});
    ...
    this.app.GET_METHOD(this.app.route.api.tNumber).subscribe((response: any) => {
     ...
     // console.log(this.attributes);
      if (response.success){
         ...
         const mm = ((today.getMonth() + 1) < 10) ? 0 + (today.getMonth() + 1).toString() : today.getMonth() + 1;
  ...
  copyAndPaste(ele) {
    const element = (document.getElementById(ele) as HTMLInputElement);
  ...
  // Function to handle Log Total
  totalLogs(total: number){
    this.logsCount = total;
  }
    ...
    if (this.attributes.id_ticket){
      this.app.GET_METHOD('addressticket/numberOfDays/?id=' + this.attributes.id_ticket).subscribe((response: any) => {
          ...
          if (response.created_date.includes('00:00:')){
            const tmp = response.created_date.substr(0, 10);
  ...
  fetchUsers() {
    this.app.GET_METHOD(this.app.route.api.fUsers).subscribe((response: Array<USER>) => {
  ...
  getConnections() {
      ...
      // if no property id or tax account kill the app
      if (!this.attributes.property_id && !this.attributes.tax_account_num) { // Kill app if either are not available..
        console.log('NO PROPERTY ID/TAX ACCOUNT');
        return;
      }
      ...
      switch (action.action) {
        case this.app.toolbarActivies.TICKET_SAVE_TRANSFER:
          ...
          if (ok) { // if there is something here perfect no route..
            ...
            const started: number = parseInt(this.attributes.started_ticket);
        ...
        case this.app.toolbarActivies.TICKET_UNARCHIVE:
          this.confirmSelection = this.app.toolbarActivies.TICKET_UNARCHIVE;
          this.confirmationName = 'UN-ARCHIVE TICKET';
          jQuery('#confirm').fadeIn('slow');
          break;
  ...
  generateLetter(value_name) {
    ...
    if (!value_name) {
      ...
      p : this.attributes.subdivision + ' ' + this.attributes.lot_num,
       f : this.attributes.full_address, m : this.attributes.msag_comm, time };
      sub = true;
    }else {
      json = {id: this.attributes.objectid,
      name : value_name,
    ...
    this.app.GET_METHOD(this.app.route.api.gLTicket + JSON_STRING).subscribe(response => {
      let name = this.attributes.objectid + '_';
      if (sub) {
  ...
  findParcelsInfo(value, property, msg: boolean, routing?: boolean) {
    ...
    const search = 'search=' + value + '&field=' + property;

    // This find method for wcad...
   // this.app.FIND_WCAD()
   // ONLY IF IS PROPERTY ID SEARCH WCAD...
    if (this.app.propertyId == property) {
     const wcadsearch = 'searchText=' + value + '&sr=4326&contains=true&searchFields=pid&layers=0&returnGeometry=true&f=pjson';
              ...
              if (routing) {
                ...
                if (!this.attributes.system_assign) { // if not available is going to get gis route...
                  ...
                  if (response.geometry.rings) { // if is not undefined do something with that information...
                    const poly = new this.app.esriPolygon(response.geometry.rings);
  ...
  getComments() {
      ...
      this.app.GET_METHOD(this.app.route.api.vtFeeds + this.attributes.id_ticket).subscribe((response: Array<FEED>) => {
          ...
          for (const com of response){
             ...
             const timeSplit = com.time_track.split(' ');
             ...
             const now = new Date();
             const track: Date = new Date(com.time_track);

             const format = this.app.FORMAT_AMPM(new Date(com.time_track));
             console.log(format);
             com.edit = false;
  ...
  delComment(id) {
    ...
    enterComment(msg: string){
      ...
      this.isLoading = true;
        ...
        // collect data
      const data = {id: this.app.account_info.user_id,
          msg, f: this.app.account_info.first_name, l: this.app.account_info.last_name, num: this.attributes.id_ticket
      };

      // from this app post data to db server..
      this.app.POST_METHOD(this.app.route.api.itFeeds, {data}).subscribe((response: any) => {
    ...
    _routeFigureAdvance(): number {
      ...
      this.attributes.system_assign = (typeof(this.attributes.system_assign) == 'string') ? JSON.parse(this.attributes.system_assign) : this.attributes.system_assign;
      const lng: number = this.attributes.system_assign.a.length;
    ...
    confirmationDecision() {
        ...
        }else if (this.confirmSelection == 2) // 2 is to copy information ticket..
        {
          ...
          this.closeConfirmation();
        }else if (this.confirmSelection === this.app.toolbarActivies.TICKET_UNARCHIVE)
        {
           // prevent
          this.stopSave = true;
          const attributes = {id_ticket: this.attributes.id_ticket,
            sentto: this.app.account_info.user_id,
            status: ''
          };
          this.attributes.status = '';
          this.attributes.sentto = this.app.account_info.user_id;
    ...
    copyInformation() {
        ...
        const poly = this.app.esriPolygon(this.selectedParcel.geometry);
      ...
      // tslint:disable-next-line:indent
			...
			const legal = this.selectedParcel.attributes.legal_desc;
			let lcount = 0;
			let lot = '';
			for (let x = legal.indexOf('LOT'); x < legal.length; x++){
				lot += legal.charAt(x);
				if (legal.charAt(x) == ' ') {
					lcount += 1;
				}
			...
			let lcount = 0;
			let lot = '';
			for (let x = legal.indexOf('LT'); x < legal.length; x++){
				lot += legal.charAt(x);
				if (legal.charAt(x) == ' ') {
					lcount += 1;
				}
		...
		// Check FOR BLOCK
			...
			let lcount = 0;
			let block = '';
			for (let x = legal.indexOf('BLOCK'); x < legal.length; x++){
			...
			let lcount = 0;
			let block = '';
			for (let x = legal.indexOf('BLK'); x < legal.length; x++){
				block += legal.charAt(x);
				if (legal.charAt(x) == ' ') {
					lcount += 1;
				}
    ...
    displayMap(data) {
        ...
        const poly = new this.app.esriPolygon(data.geometry);
    ...
    gisRouting(x, y) { // <<<<<<GET PEOPLE RESPONSIBLE FOR THE GIS ROUTE>>>>>
      ...
      this.app.GET_METHOD(this.app.route.api.gRouting + object).subscribe((response: any) => {
         console.log(response);
         if (response.success){
            const arr = (response.data.length == 1) ? JSON.parse('[' + response.data[0].staff.split(',') + ']') : response.data;
    ...
    openMessageTable(data) {
        ...
        for (const x in data.attributes) {
    ...
    // Module: Label Walk In or Utility or Ticket Archived
    // ALso: Notify the toolbar to, if available, enable un archive settings...
    isWalking() {
      ...
      this.iswalking = total;
      if (archive !== 'NONE') {
        this.app.ticketInteractionToolbar.next({archive: true, data: true})
      }
    }
