...
export class ROUTES {
   ...
   constructor() {
     this.api = {
       ...
       fUsers: "users/getUsers2/", // update fetch all users.
...
export class AppService {
  logs: Array<any> = []; //Logs for ticket keep track..
  id_ticket: number = null; // get current ticket number..
  users: Array<USER> = []; // Get a list of users from server...
  ...
  toolbarActivies: TOOL_ACTIONS = {TICKET_SAVE_TRANSFER: 1,
    ...
    QUICK_PICK: 17, TICKET_UNARCHIVE: 18
  };
  ...
  // TIcket sends commands to the toolbar...
  ...
  handleLogIn() {
    ...
    if(!this.account_info.organization_id){
      ...
      this.hisotryRoute = this.router.url;
  ...
  async esriLoadObjects(): Promise<any> {
    console.log("ESRI LOADED STARTED");
    const options = {
      url: 'https://js.arcgis.com/3.28/'
    };

    return esriLoader.loadModules(["dojo/parser","esri/Credential", 'esri/map',"esri/geometry/Extent", "esri/SpatialReference" , 'esri/config','esri/graphic', "esri/geometry/webMercatorUtils","esri/geometry/geometryEngine",
